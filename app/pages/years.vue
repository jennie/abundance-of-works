  ...
  <div>
    <div class="container-inner mx-auto py-16">
      ...
      <div v-else-if="error" class="text-center py-8 text-red-600">
        <p>Error loading years: {{ error.message }}</p>
      </div>
      <div v-else>
        <div class="divide-y divide-darkLinen">
          <div v-for="year in data?.results || []" :key="year.id">
            <NuxtLink
              :to="`/year/${year.id}`"
...
const { data, pending, error } = await useLazyAsyncData(
  `years-page-${currentPage.value}`,
  () => fetchTable(config.public.baserowYearsTableId, { 
...
useSeoMeta({
  title: 'Years - Abundance of Works',
  description: 'Browse all years in the database'
})
</script>